...
class NibbleForm {
  ...
  private $formats = array(
      'list' => array(
          ...
          'open_field' => '',
          'close_field' => '',
          'open_html' => '<div class="col-sm-12 col-sm-offset-0 col-md-6 col-md-offset-3">',
          'close_html' => "</div>",
          'open_submit' => "\n",#"<li>\n"
          'close_submit' => "\n"#"</li>\n"
      ),
  ...
  public function render() {
    ...
    $fields = '';
    $error = $this->valid ? '' : '<p class="error">Niestety wpisane dane są niepoprawne.</p>';
    ...
    <form class="form" action="$this->action" method="$this->method" enctype="multipart/form-data">
      $format->open_form
        ...
        <div class="col-sm-12 col-sm-offset-0 col-md-6 col-md-offset-3">
          <br><input class="btn " type="submit" name="submit" value="$this->submit_value" />
        </div>
        $format->close_submit
      $format->close_form
    </form>
...
class Text extends FormField {
  ...
  public function returnField($name, $value = '', $type='text') {
    ...
    return array(
        ...
        'field' => sprintf('<input class="form-control" type="%5$s" name="%1$s" id="%1$s" value="%2$s" maxlength="%3$s"%4$s />', $name, $value, $this->max_length, $class, $this->field_type),
        'html' => $this->html
    );
  }
...
FIELD;
  ...
  public function validate($val) {
    ...
    if(!isset($response[0])||$response[0] != 'true')
      $this->error[] = 'You failed to prove your humanity';
    //curl_close($ch);
